...
while [[ $THIS_TRY < $MAXIMUM_TRIES ]]; do
	...
	( set -o pipefail && xcodebuild -project CleanroomDateTime.xcodeproj -configuration Debug -scheme CleanroomDateTime-$PLATFORM -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "CleanroomDateTime-$PLATFORM-$OPERATION.log" | xcpretty )
