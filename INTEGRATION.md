...
CleanroomDateTime is **Swift 2.0**-compliant and requires **Xcode 7 beta 2 or higher** to be built.
...
Once a device-based run destination has been selected, select the “CleanroomDateTime” build scheme.
...
If all went well, the Carthage build directory should contain the file `CleanroomDateTime.framework`. If that file isn’t present, something went wrong with the build.
